...
mgr_hook_import(struct batch_request *preq)
{
	...
	if (phook->event == HOOK_EVENT_PERIODIC)
	{
		if ((phook->enabled == TRUE) && (phook->freq > 0)) {
			/* Search and delete already existing periodic hook task */
			...
			if (phook->event & HOOK_EVENT_PERIODIC) {
				if (phook->enabled == TRUE) {
					...
					if ((phook->freq > 0) && (phook->script != NULL))
						...
						sprintf(log_buffer, "periodic hook is missing information");
						log_event(PBSEVENT_ADMIN, PBS_EVENTCLASS_HOOK, LOG_INFO,
							    hookname, log_buffer);
						goto mgr_hook_set_error;
					}
 ...
 * @return	void
 */
static void post_server_periodic_hook(struct work_task *ptask) {

	int	stat = ptask->wt_aux;
	time_t next_occurance = 0;
	...
	next_occurance = (phook->last_run_timestamp + phook->freq) - time_now;
	if (next_occurance <= 0) /* If next occurance is already due then start hook ASAP */
		next_occurance = 0;
	(void)set_task(WORK_Timed, time_now + next_occurance, run_periodic_hook, phook);
...
run_periodic_hook(struct work_task *ptask)
{
	...
	if (pid != 0) {		/* The parent (main server) */
		...
		ret = server_process_hooks(PBS_BATCH_HookPeriodic, NULL, NULL, phook, HOOK_EVENT_PERIODIC, 
				   NULL, &req_ptr, hook_msg, sizeof(hook_msg), pbs_python_set_interrupt);
		if (ret == 0)
