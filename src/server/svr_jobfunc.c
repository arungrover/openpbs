		...
		if (mtype != MOVE_TYPE_MgrMv) {
			...
			if (i == 0) {
				...
				if (i == 0) {
					...
					{
						if ((comp_resc(&pque->qu_attr[QA_ATR_ResourceMin], &pjob->ji_wattr[(int)JOB_ATR_min_resc_req]) == -1) ||
							comp_resc_gt)
							i = PBSE_EXCQRESC;
						if (i == 0) {
					        	/* now check individual resources against queue or server maximum */
							chk_svr_resc_limit(&pjob->ji_wattr[(int)JOB_ATR_max_resc_req], &pque->qu_attr[QA_ATR_ResourceMax],
								             &server.sv_attr[SRV_ATR_ResourceMax],
									                    pque->qu_qs.qu_type);
 ...
 */
static int
set_select_and_place(int objtype, void *pobj, attribute *patr, attribute *pj_attr)
{
			...
			*(cvt+1) = '\0';
			...
			while (psvrl) {
				...
				if (prdefcopy && (prdefcopy->rs_flags & ATR_DFLAG_CVTSLT)) {
					...
					/* +6 = one for : = >= <= != possible quotes and null */
					cvtneed = strlen(psvrl->al_resc) +
						strlen(psvrl->al_value) + 6;
					...
					if (objtype == JOB_OBJECT) {
						presc_input = find_resc_entry(pj_attr,prdefcopy);
						if (presc_input == NULL)
					...
					if (objtype == JOB_OBJECT)
						strcat(cvt, batch_op_to_str(presc_input->rs_value.op));
					else
						strcat(cvt, "=");
...
set_chunk_sum(attribute  *pselectattr, attribute *pattr)
{
			...
			for (j=0; j<nelem; ++j) {
				for (i=0; svr_resc_sum[i].rs_def; ++i) {
					if (strcmp(svr_resc_sum[i].rs_def->rs_name, pkvp[j].kv_keyw) == 0) {
						...
						svr_resc_sum[i].rs_attr.op = pkvp[j].op;
	...
	for (i=0; svr_resc_sum[i].rs_def; ++i) {
		if (svr_resc_sum[i].rs_set) {
			...
			presc->rs_value.op = svr_resc_sum[i].rs_attr.op;
...
/**
 * @brief
 *	get_most_restrictive_spec - parses all select specifications and creates a
 *		new specification which is most restrictive in nature.
 *
 * @param[in]	multi_spec  - string with multiple select specifications
 * @param[out]  buf - newly allocated string with new select spec
 * @param[in]   mode - to find MIN or MAX resources.
 *
 * @return	Success/Failure
 *
 * @retval	PBSE_NONE: Success
 * @retval	!PBSE_NONE: In case of error.
 */
int
get_most_restrictive_spec(char *multi_spec, char **buf, int mode)
{
	char	  *sel_or = NULL;
	char	  *chunk = NULL;
	int	  rc = PBSE_NONE;
	int       nchk;
	int       nelem,x,i,j;
	int	  last_element = 0, total_elements = 0;
	int	  total_chunks = 0;
	int	  max_chunks = 0;
	struct key_value_pair *pkvp;
	resource_def	  *pdef;
	attribute   empty = {0};
	struct resc_sum *or_attrib = NULL;
	struct resc_sum *chunk_attrib = NULL;
	char *str_buf = NULL;
	int str_buf_len = 0;
	int  bufsize = 0;
	char *temp_multi_spec = NULL;
	char *id = "get_most_restrictive_spec";

	if ((multi_spec == NULL) || (buf == NULL))
		return PBSE_INTERNAL;

	temp_multi_spec = strdup(multi_spec);
	if (temp_multi_spec == NULL)
		return PBSE_SYSTEM;

	empty.at_flags |= ATR_VFLAG_SET;

	sel_or =  parse_select_or_string(temp_multi_spec);
	if (sel_or == NULL) {
		snprintf(log_buffer, sizeof(log_buffer),
			"failed to parse ORed select spec %s", temp_multi_spec);
		log_err(errno, id, log_buffer);
		free(temp_multi_spec);
		return PBSE_SYSTEM;
	}

	or_attrib = (struct resc_sum*)calloc(svr_resc_size, sizeof(struct resc_sum));
	if (or_attrib == NULL) {
		log_err(errno, id, "error allocating memory");
		free(temp_multi_spec);
		free(sel_or);
		return PBSE_SYSTEM;
	}
	chunk_attrib = (struct resc_sum *)calloc(svr_resc_size, sizeof(struct resc_sum));
	if (chunk_attrib == NULL) {
		log_err(errno, id, "error allocating memory");
		free(or_attrib);
		free(temp_multi_spec);
		free(sel_or);
	...
	while(sel_or) {
		chunk = parse_plus_spec(sel_or, &rc);
		if (rc != 0) {
			free(temp_multi_spec);
			free(or_attrib);
			free(chunk_attrib);
		...
		for (; chunk != NULL; chunk = parse_plus_spec(NULL, &rc)) {
			if (rc != 0) {
				free(temp_multi_spec);
				free(sel_or);
				free(or_attrib);
			...
			if (parse_chunk(chunk, &nchk, &nelem, &pkvp, NULL) == 0) {
				int chunk_resc_added = 0;
				total_chunks += nchk;
				for (j=0; j<nelem; ++j) {
					attribute   tmpatr;
					memset((void*)&tmpatr, 0, sizeof(attribute));
					pdef = find_resc_def(svr_resc_def, pkvp[j].kv_keyw, svr_resc_size);
					if (pdef->rs_flags & (ATR_DFLAG_RASSN|ATR_DFLAG_ANASSN|ATR_DFLAG_FNASSN)) {
						rc = pdef->rs_decode(&tmpatr, 0, 0, pkvp[j].kv_val);
						if (rc != 0) {
							free(temp_multi_spec);
							free(sel_or);
							free(or_attrib);
						...
						if (pdef->rs_type == ATR_TYPE_SIZE)
							tmpatr.at_val.at_size.atsv_num *= nchk;
						else
							tmpatr.at_val.at_long *= nchk;
						for (x=0; x<last_element; x++)
						{
							if (strcmp(chunk_attrib[x].rs_def->rs_name, pkvp[j].kv_keyw) == 0)
							{
								chunk_attrib[x].rs_def->rs_set(&chunk_attrib[x].rs_attr,&tmpatr,INCR);
								/* There is a problem in assigning op here because if there are multiple 'plus'
								* specification and same resources is mentioned with different operators in each
								* plus spec then it will result into undesirable behavior.
								* Thus assigning an EQ operator (if the operators don't match),
								* just for the sake of checking the entity resc limits.
								*/
								if (chunk_attrib[x].rs_attr.op != pkvp[j].op)
								{
									sprintf(log_buffer, "operator conflict encountered ,using EQ operator");
									log_event(PBSEVENT_DEBUG, PBS_EVENTCLASS_JOB,
										    LOG_INFO, msg_daemonname,
										    log_buffer);
									chunk_attrib[x].rs_attr.op = EQ;
								}
								chunk_resc_added = 1;
								break;
							}
						}
						if (chunk_resc_added == 0)
						{
							chunk_attrib[x].rs_def = pdef;
							chunk_attrib[x].rs_def->rs_set(&chunk_attrib[x].rs_attr,&tmpatr,SET);
							chunk_attrib[x].rs_attr.op = pkvp[j].op;
							last_element++;
						}
					}
					chunk_resc_added = 0;
		...
		for (x=0;x<last_element;x++)
		{
			...
			{
				if (or_attrib[i].rs_def == chunk_attrib[x].rs_def)
				{
					if ((or_attrib[i].rs_def->rs_flags & ATR_DFLAG_ANASSN) ||
					    (or_attrib[i].rs_def->rs_flags & ATR_DFLAG_RASSN) ||
					    (or_attrib[i].rs_def->rs_flags & ATR_DFLAG_FNASSN)) {
						int comp = chunk_attrib[x].rs_def->rs_comp(&or_attrib[i].rs_attr, &chunk_attrib[x].rs_attr);
						/* compare or_attrib and chunk_attrib when operators in both these attribs are same */
						if (or_attrib[i].rs_attr.op == chunk_attrib[x].rs_attr.op) {
							if (((mode == MAX_RESC_SPEC) && (comp < 0)) || ((mode == MIN_RESC_SPEC) && (comp > 0))) {
								chunk_attrib[x].rs_def->rs_set(&or_attrib[i].rs_attr, &chunk_attrib[x].rs_attr, SET);
								or_attrib[i].rs_attr.op = chunk_attrib[x].rs_attr.op;
							}
						}
						else {
							/* When mode is to find MAX_RESC_SPEC and chunk_attrib truns out to be greater in value than or_attrib then
							 * make sure that or_attrib does not have GT/GE operator set because these operators can result into maximum
							 * resource allocation. Also make sure that if value of or_attrib is greater than that of chunk_attrib then 
							 * chunk_attrib must have GT/GE operator in order to replace or_attrib.
							 */
							if ((mode == MAX_RESC_SPEC)  && (((comp < 0) && (or_attrib[i].rs_attr.op != GT && or_attrib[i].rs_attr.op != GE)) ||
											((comp > 0) && (chunk_attrib[x].rs_attr.op == GT || chunk_attrib[x].rs_attr.op == GE)))) {
								chunk_attrib[x].rs_def->rs_set(&or_attrib[i].rs_attr, &chunk_attrib[x].rs_attr, SET);
								or_attrib[i].rs_attr.op = chunk_attrib[x].rs_attr.op;
							}
							/* This case is exactly opposite of the case described above */
							else if ((mode == MIN_RESC_SPEC) && (((comp > 0) && (or_attrib[i].rs_attr.op != LT && or_attrib[i].rs_attr.op != LE)) ||
											    ((comp < 0 ) && (chunk_attrib[x].rs_attr.op == LT || chunk_attrib[x].rs_attr.op == LE)))) {
								chunk_attrib[x].rs_def->rs_set(&or_attrib[i].rs_attr, &chunk_attrib[x].rs_attr, SET);
								or_attrib[i].rs_attr.op = chunk_attrib[x].rs_attr.op;
							}
						}
			...
			if (or_resc_added == 0)
			{
				or_attrib[i].rs_def = chunk_attrib[x].rs_def;
				or_attrib[i].rs_def->rs_set(&or_attrib[i].rs_attr, &chunk_attrib[x].rs_attr, SET);
				or_attrib[i].rs_attr.op = chunk_attrib[x].rs_attr.op;
				total_elements++;
			}
			or_resc_added = 0;
			chunk_attrib[x].rs_def->rs_set(&chunk_attrib[x].rs_attr,&empty,SET);
		}
		last_element = 0;
		total_chunks = 0;
		free(sel_or);
	...
	*buf = (char *) calloc(1,1);
	...
	for (i = 0; i < total_elements; i++) {
		svrattrl *tmprl;
		int str_len = 0;
		if (or_attrib[i].rs_def->rs_encode(&or_attrib[i].rs_attr,NULL, or_attrib[i].rs_def->rs_name, NULL, ATR_ENCODE_CLIENT, &tmprl) > 0) {
			str_len = strlen(or_attrib[i].rs_def->rs_name) + strlen(batch_op_to_str(or_attrib[i].rs_attr.op)) 
				+ strlen(tmprl->al_atopl.value) + 2;
			if (str_buf_len < str_len)
			{
				char *temp = NULL;
				temp = (char *)realloc(str_buf, str_len);
				if (temp == NULL)
				{
					free(str_buf);
					free(*buf);
			...
			sprintf (str_buf,"%s%s%s:", or_attrib[i].rs_def->rs_name, batch_op_to_str(or_attrib[i].rs_attr.op), tmprl->al_atopl.value);
			if (pbs_strcat (buf, &bufsize, str_buf) == NULL) {
			        free (*buf);
				free(temp_multi_spec);
				free(tmprl);
				free(str_buf);
...
/**
 * @brief
 * 	parse_each_chunk -  decode a selection specification,  and produce the
...
static int
parse_each_chunk (char **outbuf1, char *val, int validate_resource_exist, pbs_queue *pque)
{
	char        *chunk;
	int          i;
	int          firstchunk;
			...
			if (nelem > 0) {
				...
				for (i=0; i<nelem; ++i) {
					...
					if (strcat_grow(&outbuf, &pc, &bufsz, pkvp[i].kv_keyw) == -1)
						return PBSE_SYSTEM;
					strcat(pc,batch_op_to_str(pkvp[i].op));
...
/**
 * @par
 * make_or_selspec - This function parses through the select spec string and 
 *	if there are multiple ORed select specification it returns 
 *	each one of them everytime it is called until there is no
 *	specification left.
 *
 *	@param	patrl - (not used)
 *	@param	pselect - pointer to the select specification
 *	@param	pque	- used to obtain queue defaults
 *	@param	psched
 *
 *	@return	int
 */
static int
make_or_selspec(attribute *patrl, resource *pselect,
	        pbs_queue *pque, attribute *psched)
{
	...
	size_t sel_size = 0;
	char *id = "make_or_selspec";

	val = strdup(pselect->rs_value.at_val.at_str);
	...
	if (sel_or == NULL) {
		snprintf(log_buffer, sizeof(log_buffer), "failed to parse ORed select spec %s", val);
		log_err(errno, id, log_buffer);
		free(val);
		return PBSE_SYSTEM;
	}
	while (sel_or) {
		ret = parse_each_chunk(&outbuf, sel_or, validate_resource_exist, pque);
		if (ret == 0 ) {
			if(outbuf != NULL && *outbuf != '\0') {
				if (sched_select == NULL) {
					sched_select = (char *)calloc(1,strlen(outbuf) + PBS_STRCAT_GROW_MIN);
					if (sched_select == NULL)
						return PBSE_SYSTEM;
					strcat(sched_select,outbuf);
				}
				else {
					char * place = &sched_select[strlen(sched_select)];
					strcat_grow(&sched_select,&place,&sel_size, "||");
					place += 2;
					strcat_grow(&sched_select,&place,&sel_size, outbuf);
					place = &sched_select[strlen(sched_select)];
				}
			}
		}
		else {
	...
	free(sel_or);
	free_str(psched);
	(void)decode_str(psched, NULL, NULL, sched_select);
...
/**
 * @par
 * make_simple_selspec - decodes a simple select (!multiple) specification,  and produce the
 *	the "schedselect" attribute which contains any default resources
 *	missing from the chunks in the select spec.
 *	Also translates the value of any boolean resource to the "formal"
 *	value of "True" or "False" for the Scheduler who needs to know it
 ...
 */
static int
make_simple_selspec(attribute *patrl, resource *pselect,
	pbs_queue *pque, attribute *psched)
{
	char        *val;
	int          validate_resource_exist = 0;
	...
	if (pque == NULL || pque->qu_qs.qu_type == QTYPE_Execution)
		validate_resource_exist = 1;
	ret = parse_each_chunk(&outbuf, val, validate_resource_exist, pque);
	if (ret == 0) {
		free_str(psched);
...
/**
 * @par
 *  make_schedselect - This function checks if the select specification had
 *		multiple select specifications or not and accordingly calls
 *		make_or_selspec/make_simple_selspec.
 *
 *	@param	patrl - (not used)
 * 	@param	pselect - pointer to the select specification
 * 	@param	pque	- used to obtain queue defaults
	...
	pbs_queue *pque, attribute *psched)
{
	...
	if (strstr(val,"||") != NULL)
		return (make_or_selspec(patrl, pselect, pque, psched));
	else
		return (make_simple_selspec(patrl, pselect, pque, psched));
}
...
set_resc_deflt(void *pobj, int objtype, pbs_queue *pque)
{
	...
	if ((presc == NULL) || ((presc->rs_value.at_flags & ATR_VFLAG_SET)==0)) {
		if (objtype == JOB_OBJECT) {
			if ((rc = set_select_and_place(objtype, pobj, pdest, &pjob->ji_wattr[(int)JOB_ATR_resource])) != 0)
				return rc;
		}
		else {
			if ((rc = set_select_and_place(objtype, pobj, pdest, NULL)) != 0)
				return rc;
		}
	}
	...
	if (presc) {
		if ((rc = make_schedselect(pdest, presc , pque, psched)) == 0) {
			if (strstr(psched->at_val.at_str,"||") != NULL)
			{
				char *first_spec = NULL;
				char * temp = NULL;
				char *select_copy = strdup(psched->at_val.at_str);
				temp = psched->at_val.at_str;
				if (select_copy == NULL)
					return PBSE_SYSTEM;
				rc = get_most_restrictive_spec(psched->at_val.at_str, &cumulative_spec, MAX_RESC_SPEC);
				if (rc) {
					free(select_copy);
					return rc;
				}
				else {
					psched->at_val.at_str = cumulative_spec;
					/* set JOB_ATR_max_resc_req attribute in job. This attribute is used to 
					 * check max_queued resource limits.
					 */
					rc = set_chunk_sum(psched, &pjob->ji_wattr[(int)JOB_ATR_max_resc_req]);
					psched->at_val.at_str = temp;
					free(cumulative_spec);
				}
				rc = get_most_restrictive_spec(psched->at_val.at_str, &cumulative_spec, MIN_RESC_SPEC);
				if (rc) {
					free(select_copy);
					return rc;
				}
					...
					psched->at_val.at_str = cumulative_spec;
					/* set JOB_ATR_min_resc_req attribute in job. This attribute is used to 
					 * check resource_min resource limits.
					 */
					rc = set_chunk_sum(psched, &pjob->ji_wattr[(int)JOB_ATR_min_resc_req]);
					psched->at_val.at_str = temp;
					free(cumulative_spec);
				}
				first_spec = parse_select_or_string(select_copy);
				if (first_spec == NULL) {
					free(select_copy);
					return PBSE_SYSTEM;
				}
				/* Set the resource request of the job according to first select specification.
				 * this is needed to sort jobs based on their first select specification in job_sort_formula
				 */
				psched->at_val.at_str = first_spec;
				...
				rc = set_chunk_sum(psched, pdest);
			if (objtype == JOB_OBJECT)
				/* Set RequestedSpec same as SchedSelect */
				job_attr_def[(int)JOB_ATR_RequestedSpec].at_set(&pjob->ji_wattr[(int)JOB_ATR_RequestedSpec], psched, SET);
		}
