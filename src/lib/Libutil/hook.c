...
set_hook_alarm(hook *phook, char *newval, char *msg, size_t msg_len)
{
	...
	if ((phook->event == HOOK_EVENT_PERIODIC) && (alarm > (phook->freq - 5))) {
		snprintf(msg, msg_len-1,
			"Can't set hook alarm value: Server periodic hook's alarm value"\
			" must be at least 5 seconds less than hook frequency", __func__, 
...
set_hook_freq(hook *phook, char *newval, char *msg, size_t msg_len)
{
	...
	if ((phook->event == HOOK_EVENT_PERIODIC) && (freq < (phook->alarm + 5))) {
		snprintf(msg, msg_len-1,
			"%s: Can't set hook freq value: hook frequency must be at least"\
			" 5 seconds more than hook alarm", __func__);
		return (1);
		}
	phook->freq = freq;
	return (0);
